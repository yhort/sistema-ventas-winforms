...
{
    public partial class Frm_ModuloCredito : Form
    {
        public Frm_ModuloCredito()
        {
            InitializeComponent();
        }
        ...
        private void Configurar_listView()
        {
            ...
            lis.Columns.Add("ID Notacred", 80, HorizontalAlignment.Left); //0
            lis.Columns.Add("Id_Doc", 400, HorizontalAlignment.Left); //1
            lis.Columns.Add("Fecha_Credito", 80, HorizontalAlignment.Left); //2
            lis.Columns.Add("Nom_Cliente", 90, HorizontalAlignment.Right); //3
            lis.Columns.Add("Total_Cre", 90, HorizontalAlignment.Right); //4
        ...
        private void Agregar_Productos_alCarrito(string xidprod, string xnomprod,  double xcant, double xprecio, double ximporte , double xtotalcred, double xsaldopendiente)
        {
            try
            {
                if (lsv_Det.Items.Count == 0)
                {
                    ...
                    item.SubItems.Add(xtotalcred.ToString("###0.00"));
                    item.SubItems.Add(xsaldopendiente.ToString("###0.00"));
        ...
        private void btn_Nuevo_buscarProd_Click(object sender, EventArgs e)
        {
            ...
            Frm_Listado_IngresoCredito pro = new Frm_Listado_IngresoCredito();
            ...
            if (pro.Tag.ToString() == "A")
            {
                ...
                double _cant = Convert.ToDouble(pro.lbl_Cant_Abono.Text);

                double _precio = Convert.ToDouble(pro.lbl_preCom.Text);
                double _importe = Convert.ToDouble(pro.lbl_preCom.Text);

                double _totalcred = Convert.ToDouble(pro.lbl_totalcred.Text);
                double _saldopendiente = Convert.ToDouble(pro.lbl_saldopendiente.Text);

                Agregar_Productos_alCarrito(_idprod.Trim(), _nomprod, _cant, _precio, _importe, _totalcred, _saldopendiente);
                txt_IdComp.Text = RN_TipoDoc.RN_NroID(12);
